...
export const GET = async (req: NextRequest) => {
  try {
    ...
    if (!accessTokenRes)
      return new Response(
        JSON.stringify({
          error: `Google login failed. accessTokenRes Res: ${JSON.stringify(accessTokenRes)}`,
        }),
        { status: 401 },
      )
    ...
    if (!data)
      return new Response(
        JSON.stringify({
          error: `Google login failed. exchangeCodeForAccessToken Res: ${JSON.stringify(
            accessTokenRes,
          )}; getUserInfo Res: ${JSON.stringify(data)} `,
        }),
        { status: 401 },
      )
