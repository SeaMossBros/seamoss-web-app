...
const ToolTip = ({ children, title, width = 'auto', opacity = 0.9, delay = 900 }: ToolTipProps) => {
  ...
  }, [showTooltip, delay, shouldDisplay])
  ...
  const renderToolTipText = () => {
    ...
    return title.split('\\n').map((line, index, array) => {
      ...
      const parts = line.split(/<u>|<\/u>/).map((part, partIndex) => {
        ...
        if (partIndex % 2 === 1) {
          return (
            <u key={partIndex} style={{ textUnderlineOffset: '3px' }}>
              {part}
            </u>
          )
        }
